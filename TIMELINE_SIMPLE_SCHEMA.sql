-- Simplified Timeline Events Schema (no dependencies on document_signatures)
...
DROP POLICY IF EXISTS "Users can update timeline events" ON timeline_events;
DROP POLICY IF EXISTS "Users can delete timeline events" ON timeline_events;

-- Create new policies with broader access

-- Users can view events for their own properties
...
BEGIN
  ...
  SELECT
    ...
    NEW.id,
    'Propiedad publicada',
    'La propiedad "' || NEW.address || ', ' || NEW.district || '" ha sido publicada en el sistema',
    'property_listed',
    'completed',
    up.full_name
